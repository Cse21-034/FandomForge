...
export default function CollectionPage() {
  ...
  // Check access after load
  useEffect(() => {
    if (!collection || !isAuthenticated || !collectionId) return;
            ...
            {activeItem ? (
              <div className="rounded-2xl overflow-hidden bg-card border border-card-border">
                {activeItem.itemType === "video" && (
                  <VideoPlayer
                    isLocked={!canViewItem(activeItem)}
                    videoUrl={canViewItem(activeItem) ? activeItem.videoUrl : undefined}
                    thumbnail={activeItem.thumbnailUrl}
                    onUnlock={handleUnlock}
                  />
                )}

                {activeItem.itemType === "image" && canViewItem(activeItem) && (
                ...
                {activeItem.itemType === "text" && canViewItem(activeItem) && (
          ...
          <div className="lg:col-span-1">
            ...
            <div className="space-y-2">
              {items.map((item: any, index: number) => {
                ...
                const isViewable = canViewItem(item);
